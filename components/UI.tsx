...
export const Button: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'primary' | 'secondary' | 'danger' | 'ghost', icon?: LucideIcon }> = ({ className = '', variant = 'primary', icon: Icon, children, onClick, ...props }) => {
  const base = "inline-flex items-center justify-center px-4 py-2 border text-sm font-bold rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all active:scale-95";
  const variants = {
...
export const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement> & { label?: string, error?: string }> = ({ label, className = '', error, ...props }) => (
  <div className="mb-4">
    ...
    <input
      className={`shadow-none focus:ring-2 focus:ring-brand-accent focus:border-brand-accent block w-full sm:text-sm border-gray-200 rounded-xl p-3 border transition-all ${error ? 'border-red-300 ring-red-100 ring-2' : ''} ${className}`}
      {...props}
    />
...
export const Select: React.FC<React.SelectHTMLAttributes<HTMLSelectElement> & { label?: string }> = ({ label, children, className = '', ...props }) => (
  <div className="mb-4">
    {label && <label className="block text-xs font-black text-slate-500 mb-2 uppercase tracking-wider">{label}</label>}
    <select
      className={`shadow-none focus:ring-2 focus:ring-brand-accent focus:border-brand-accent block w-full sm:text-sm border-gray-200 rounded-xl p-3 border transition-all bg-white appearance-none ${className}`}
      {...props}
    >
      {children}
    </select>
  ...
  return (
    <div className="fixed z-50 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block align-bottom bg-white rounded-[32px] text-right overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl sm:w-full animate-in fade-in zoom-in duration-300">
          <div className="bg-white px-8 pt-8 pb-8">
            <h3 className="text-xl font-black text-slate-900 mb-6" id="modal-title">{title}</h3>
            <div className="mt-2 text-right">{children}</div>
          </div>
          <div className="bg-slate-50 px-8 py-4 sm:flex sm:flex-row-reverse border-t border-slate-100">
            <Button variant="secondary" onClick={onClose} className="w-full sm:w-auto">إغلاق</Button>
          </div>
        </div>
      </div>
    </div>
...
export const Badge: React.FC<{ color: string; children: React.ReactNode; className?: string }> = ({ color, children, className = '' }) => {
    ...
    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wider ${classes} ${className}`}>
        {children}
      </span>
    );
  };
